...
void Allocate(int endindex,int j1, int j2); //added a parameter

int Hashed[1000];

float MachineSearch(int MachineNum,float TimeConstraint);

int LeastCheck(float *LeastArray,int j1, int j2); //added a parameter
...

{
    ...
    int j=Mac-1,random_var,my_count=0;
    int j1, j2;
     while(G_tail>0)
                   ...
                   if(my_count==10)
                    {
                      /*random_var=rand()%2;
                        ...
                        if( j>=Mac || j<2)
                            j=Mac-1;*/
                        my_count=0;
                        j1 = (rand()+my_time->tm_sec)%Mac;
                        j2 = (my_time->tm_sec)%Mac;
                        while(j1==j2)
                            j2=(rand()+my_time->tm_sec)%Mac;
                        //printf("%d %d %d",j1,j2,Mac);
...
void Allocate(int endindex, int j1, int j2) //to Allocate to a machine

{
   // int endindex=0;
...

     {
         ...
         find = LeastCheck(LeastMacArray,j1,j2);
...
int LeastCheck(float *LeastArray,int j1, int j2)
