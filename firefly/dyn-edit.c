...
void ProcAlloc(int );
int CurrentTask;
void Allocate(int endindex,int res[]);
int Hashed[100];
...
for(i=0;i<2;i++)
{
    for(k=0;k<10;k++)
    {
        printf("ff no : %d pos val :%f sequence : %d \n",i,d[i].posval[k],d[i].seq[k]);
    }
    printf("\n");
}
...
void ProcAlloc(int nmac)

{
    int j=0;
    int res[nmac-1],len=sizeof(seq)/seq[0],i=0,val=0;
     while(G_tail>0)
...
void Allocate(int endindex,int res[]) //to Allocate to a machine

{
...

     {
         ...
         find = LeastCheck(LeastMacArray,res);
...
int LeastCheck(float *LeastArray,int res[])
